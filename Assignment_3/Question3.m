...
x0 = [-5;10];
e = 0.01;
gk = [0.1 0.02 0.15 0.17];
sk = [15 15 2 1.75];

for n=1:length(gk)
    q = steepdscproj(gradf, x1, x2, x0, e, gk(n), sk(n));

    figure;
    fcontour(f);
    hold on;
    plot(q.x(1,:), q.x(2,:), LineStyle='--', Color='r', Marker='*', MarkerEdgeColor='b');
    title(sprintf('Convergence path\nProjected Steepest Descent with x_0=(%g,%g), ε=%g, γ_k=%g and s_k=%g (constants)', x0(1), x0(2), e, gk(n), sk(n)), FontSize=15);
...
x0 = [-5;0];
q = steepdscproj(gradf, x1, x2, x0, e, gk(1), sk(1));

figure;
fcontour(f);
hold on;
plot(q.x(1,:), q.x(2,:), LineStyle='--', Color='r', Marker='*', MarkerEdgeColor='b');
title(sprintf('Convergence path\nProjected Steepest Descent with x_0=(%g,%g), ε=%g, γ_k=%g and s_k=%g (constants)', x0(1), x0(2), e, gk(1), sk(1)), FontSize=15);
xlabel('x_1', FontWeight='bold', FontSize=14);
ylabel('x_2', FontWeight='bold', Rotation=0,FontSize=14);
legend('f(x)', 'Iterations');
