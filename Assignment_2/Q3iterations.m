...
title({'f(xk) vs k','Newton with (x0,y0)=(0,0) and γk=0.9 (constant)'});
...
title({'f(xk) vs k','Newton with (x0,y0)=(-1,-1) and γk=0.9 (constant)'});
xlabel('k (iterations)', FontWeight='bold');
ylabel('f(xk)', FontWeight='bold', Rotation=0);
...
title({'f(xk) vs k','Newton with (x0,y0)=(0,0) and γk that minimizes f(xk+γk*dk)'});
xlabel('k (iterations)', FontWeight='bold');
ylabel('f(xk)', FontWeight='bold', Rotation=0);

figure(5);
plot(1:qb2.k, double(subs(f, {x,y}, {qb2.xv(1,:),qb2.xv(2,:)})), LineStyle='--', Color='r', Marker='*', MarkerEdgeColor='b');
title({'f(xk) vs k','Newton with (x0,y0)=(-1,-1) and γk that minimizes f(xk+γk*dk)'});
xlabel('k (iterations)', FontWeight='bold');
ylabel('f(xk)', FontWeight='bold', Rotation=0);
...
title({'f(xk) vs k','Newton with (x0,y0)=(0,0) and γk chosen by the Armijo rule'});
xlabel('k (iterations)', FontWeight='bold');
ylabel('f(xk)', FontWeight='bold', Rotation=0);

figure(8);
plot(1:qc2.k, double(subs(f, {x,y}, {qc2.xv(1,:),qc2.xv(2,:)})), LineStyle='--', Color='r', Marker='*', MarkerEdgeColor='b');
title({'f(xk) vs k','Newton with (x0,y0)=(-1,-1) and γk chosen by the Armijo rule'});
xlabel('k (iterations)', FontWeight='bold');
ylabel('f(xk)', FontWeight='bold', Rotation=0);
